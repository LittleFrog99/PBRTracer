...
void BVH::radixSort(vector<MortonPrimitive> *v) {
    ...
    if (nPasses & 1) swap(*v, tempVector);
}

// BVH Method Definitions
BVH::BVH(vector<shared_ptr<Primitive>> p,
                   int maxPrimsInNode, SplitMethod splitMethod)
    : maxPrimsInNode(min(255, maxPrimsInNode)),
      splitMethod(splitMethod),
      primitives(move(p)) {
    ProfilePhase _(Stage::AccelConstruction);
    if (primitives.empty()) return;
    // Build BVH from _primitives_

    // Initialize _primitiveInfo_ array for primitives
    vector<BVHPrimitiveInfo> primitiveInfo(primitives.size());
    ...
    LOG(INFO) << STRING_PRINTF("BVH created with %d nodes for %d "
    ...
    nodes = Memory::allocAligned<LinearBVHNode>(totalNodes);
    int offset = 0;
...
BVH::BVHBuildNode *BVH::recursiveBuild(MemoryArena &arena, vector<BVHPrimitiveInfo> &primitiveInfo, int start,
                                       int end, int *totalNodes, vector<shared_ptr<Primitive>> &orderedPrims) {
    CHECK_NE(start, end);
    ...
    for (int i = start; i < end; ++i)
        bounds = unionOf(bounds, primitiveInfo[i].bounds);
    ...
    if (nPrimitives == 1) {
        ...
        for (int i = start; i < end; ++i)
            centroidBounds = unionOf(centroidBounds, primitiveInfo[i].centroid);
        ...
        if (centroidBounds.pMax[dim] == centroidBounds.pMin[dim]) {
            ...
            case SplitMethod::Middle: {
                ...
                BVHPrimitiveInfo *midPtr = partition(&primitiveInfo[start], &primitiveInfo[end - 1] + 1,
            ...
            case SplitMethod::EqualCounts: {
                ...
                mid = (start + end) / 2;
                nth_element(&primitiveInfo[start], &primitiveInfo[mid], &primitiveInfo[end - 1] + 1,
            ...
            default: {
                ...
                if (nPrimitives <= 2) {
                    ...
                    for (int i = start; i < end; ++i) {
                        ...
                        CHECK_LT(b, nBuckets);
                        buckets[b].count++;
                        buckets[b].bounds = unionOf(buckets[b].bounds, primitiveInfo[i].bounds);
                    }
...
BVH::BVHBuildNode *BVH::HLBVHBuild(MemoryArena &arena, const vector<BVHPrimitiveInfo> &primitiveInfo,
    ...
    *totalNodes = atomicTotal;
    ...
    vector<BVHBuildNode *> finishedTreelets;
...
BVH::BVHBuildNode *BVH::buildUpperSAH(MemoryArena &arena,
    ...
    BVHBuildNode **pmid = partition(&treeletRoots[start], &treeletRoots[end - 1] + 1,
...
bool BVH::intersect(const Ray &ray, SurfaceInteraction *isect) const {
    ...
    ProfilePhase p(Stage::AccelIntersect);
    bool hit = false;
...
bool BVH::intersectP(const Ray &ray) const {
    if (!nodes) return false;
    ProfilePhase p(Stage::AccelIntersectP);
