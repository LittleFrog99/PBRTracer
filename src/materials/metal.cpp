...
MetalMaterial * MetalMaterial::create(const TextureParams &mp) {
    static Spectrum copperN = Spectrum::fromSampled(CopperWavelengths, CopperN, CopperSamples);
    shared_ptr<Texture<Spectrum>> eta = mp.getTexture("eta", copperN);
    static Spectrum copperK = Spectrum::fromSampled(CopperWavelengths, CopperK, CopperSamples);
    shared_ptr<Texture<Spectrum>> k = mp.getTexture("k", copperK);
    shared_ptr<Texture<float>> roughness = mp.getTexture("roughness", .01f);
    shared_ptr<Texture<float>> uRoughness =  mp.getFloatTextureOrNull("uroughness");
    shared_ptr<Texture<float>> vRoughness = mp.getFloatTextureOrNull("vroughness");
...
const float MetalMaterial::CopperWavelengths[CopperSamples] = {
...
const float MetalMaterial::CopperN[CopperSamples] = {
...
const float MetalMaterial::CopperK[CopperSamples] = {
