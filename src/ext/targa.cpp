...
tga_result tga_read(tga_image *dest, const char *filename) {
    ...
    if (fp == nullptr) return TGAERR_FOPEN;
...
#define READ16(dest)                                         \
    ...
    dest->image_id = nullptr;
    dest->color_map_data = nullptr;
    dest->image_data = nullptr;
    ...
    if (dest->image_id_length > 0) {
        ...
        if (dest->image_id == nullptr) BARF(TGAERR_NO_MEM);
    ...
    if (dest->color_map_type == TGA_COLOR_MAP_PRESENT) {
        ...
        if (dest->color_map_data == nullptr) BARF(TGAERR_NO_MEM);
    ...
    if (dest->image_data == nullptr) BARF(TGAERR_NO_MEM);
...
static void init_tga_image(tga_image *img, uint8_t *image, const uint16_t width,
    ...
    img->image_id = nullptr;
    img->color_map_data = nullptr;
...
tga_result tga_color_unmap(tga_image *img) {
    ...
    if (tmp == nullptr) return TGAERR_NO_MEM;
 ...
 * R_TO_L, etc).  Returns nullptr if the pixel is out of range.
 */
uint8_t *tga_find_pixel(const tga_image *img, uint16_t x, uint16_t y) {
    if (x >= img->width || y >= img->height) return nullptr;
 ...
 * be set to nullptr if not wanted.  Returns TGAERR_PIXEL_DEPTH if a stupid
...
tga_result tga_desaturate(tga_image *img, const int cr, const int cg,
    ...
    for (src = img->image_data;
        ...
        uint8_t b, g, r;
        (void)tga_unpack_pixel(src, img->pixel_depth, &b, &g, &r, nullptr);
 ...
 * tga_image, if they're not already nullptr.
 */
void tga_free_buffers(tga_image *img) {
    if (img->image_id != nullptr) {
        free(img->image_id);
        img->image_id = nullptr;
    }
    if (img->color_map_data != nullptr) {
        free(img->color_map_data);
        img->color_map_data = nullptr;
    }
    if (img->image_data != nullptr) {
        free(img->image_data);
