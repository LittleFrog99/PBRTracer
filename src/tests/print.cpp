...
TEST(StringPrintf, Basics) {
    EXPECT_EQ(STRING_PRINTF("Hello, world"), "Hello, world");
    EXPECT_EQ(STRING_PRINTF("x = %d", 5), "x = 5");
    EXPECT_EQ(STRING_PRINTF("%f, %f, %f", 1., 1.5, -8.125),
              "1, 1.5, -8.125");
}
...
TEST(OperatorLeftShiftPrint, Basics) {
    ...
    {
        std::ostringstream os;
        Transform t = Transform::translate(Vector3f(-1.25, 3.5, 7.875)) * Transform::scale(2., -8., 1);
        os << t;
        EXPECT_EQ(
            os.str(),
            "t=[ [ 2, 0, 0, -1.25 ] "
